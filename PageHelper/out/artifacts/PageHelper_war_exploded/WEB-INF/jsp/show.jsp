    ...
    <script type="text/javascript">
        ...
        function firstPage(){
            ...
            if(1==pageNum){
                ...
                window.location.href="<%=request.getContextPath()%>/list?pageNum="+1;
            }
        }
        function prePage(){
            var pageNum=document.getElementById("pageNum").value;
            ...
            if (pageNum < 1){
                ...
                window.location.href="<%=request.getContextPath()%>/list?pageNum="+pageNum;
            }
        }
        function nextPage(){
            var pageNum=document.getElementById("pageNum").value;
            ...
            if (pageNum > totalPage) {
                alert("已经是最后一页！")
                window.location.href="<%=request.getContextPath()%>/list?pageNum="+totalPage;
...
<table width="100%" border=1>
    ...
    <c:forEach items="${student.getList()}" var="s">
        <tr>
            <td>${s.num}</td>
            <td>${s.name}</td>
            <td>${s.sex}</td>
